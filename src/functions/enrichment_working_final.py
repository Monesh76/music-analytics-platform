...
Correctly handles Eventarc Pub/Sub triggers and Claude LLM integration.
"""
    ...
    try:
        ...
        if hasattr(cloud_event, 'data') and cloud_event.data:
            if isinstance(cloud_event.data, str):
                try:
                    # Decode base64 data (Eventarc format)
                    message_data = base64.b64decode(cloud_event.data).decode('utf-8')
                    logger.info("✅ SUCCESS: Decoded base64 Eventarc Pub/Sub message data")
                except Exception as e:
                    logger.error(f"❌ FAILED to decode base64 data: {e}")
                    return
            else:
                logger.error(f"❌ UNEXPECTED DATA TYPE: {type(cloud_event.data)}")
                return
