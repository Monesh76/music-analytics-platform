"""
Correct Cloud Function for Claude LLM enrichment of music events.
Handles Eventarc Pub/Sub triggers correctly.
"""
...
def enrich_music_event(cloud_event) -> None:
    """
    Cloud Function triggered by Eventarc Pub/Sub to enrich music events with Claude LLM.
    """
    ...
    try:
        ...
        # Eventarc Pub/Sub triggers send the message data directly in cloud_event.data
        # The data is base64 encoded JSON string
        message_data = None
        
        if hasattr(cloud_event, 'data') and cloud_event.data:
            # For Eventarc Pub/Sub triggers, cloud_event.data is the base64 encoded message
            if isinstance(cloud_event.data, str):
                try:
                    # Decode base64 data
                    message_data = base64.b64decode(cloud_event.data).decode('utf-8')
                    logger.info("Successfully decoded base64 Eventarc Pub/Sub message data")
                except Exception as e:
                    logger.error(f"Failed to decode base64 data: {e}")
                    return
            else:
                logger.error("Eventarc Pub/Sub message is not a string")
                logger.error(f"Message structure: {cloud_event.data}")
                return
        
        if not message_data:
            logger.error("No data in Eventarc Pub/Sub message")
            return
